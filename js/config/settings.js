...
function openSettingsPanel() {
  ...
  addNumberControl(content, {
    label: 'Sounds Volume:',
    settingKey: 'volume',
    min: AUDIO.MIN_VOLUME,
    max: AUDIO.MAX_VOLUME,
    step: AUDIO.STEP_VOLUME,
    onChange: (value) => {
      if(window.audioManager?.setVolume){
 ...
 * @param {string} theme - Theme name ('Green' or 'Orange' or 'Red')
 */
export function applyTheme(theme) {
  ...
  if (normalized === 'Green') {
    ...
    terminal.style.background = '#1a0000';
    terminal.style.backgroundImage = 'radial-gradient(ellipse, #330000 0%, #050000 90%)';
    if (terminalInput) terminalInput.style.color = '#ff3333';
    if (terminalOutput) terminalOutput.style.color = '#ff3333';
    if (inputPrefix) inputPrefix.style.color = '#ff3333';
    document.documentElement.style.setProperty('--cursor-color', '#ff3333');
  }
}
